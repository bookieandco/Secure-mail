...
function policyObject(object: ProviderObject) {
  return {
    ...
    version: object.versionId ?? object.etag,
    checksumSha256: object.etag,
  };
}
...
function assertAuthorized(context: StorageServiceContext, capability: 'READ' | 'WRITE' | 'DELETE' | 'LIST', key: string, object: ProviderObject | null, now: Date): void {
  if (context.namespace.id !== context.policy.namespaceId || context.namespace.id !== context.grant.namespaceId) {
    throw new Error('namespace_policy_mismatch');
  }
  if (!context.policy.encryptionRequired) throw new Error('encryption_policy_required');
  const decision = authorizeStorageRequest(context.policy, context.grant, object ? policyObject(object) : null, {
    principalId: context.grant.principalId,
...
export async function putObject(context: StorageServiceContext, input: PutObjectInput, now: Date): Promise<ProviderObject> {
  ...
  assertAuthorized(context, 'WRITE', input.key, null, now);
  if (context.namespace.quotaBytes < input.body.byteLength) throw new Error('storage_quota_exceeded');
